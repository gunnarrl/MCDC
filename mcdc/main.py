...
def _compute_material_row(
    ...
    mcdc,
    data,
    locate_particle_func,  # pass locate_particle() to avoid import issues
):
    """
    Compute material IDs for a single row of pixels (JIT-compiled).
    ...
    for j in range(n_second):
        ...
        if locate_particle_func(particle_arr, mcdc, data):
    ...
    for t in time:
        ...
        for i in range(pixels[0]):
            row_materials = _compute_material_row(
                ...
                mcdc,
                data,
                geometry.locate_particle,
            )
            material_ids[i, :] = row_materials

            # Update progress bar
            progress = int((i + 1) / pixels[0] * 100)
            if progress > last_progress:
                last_progress = progress
                bar_width = 28
                filled = int(progress / 100 * bar_width)
                sys.stdout.write(f"\r Visualizing: [{'=' * filled:<28}] {progress}%")
                sys.stdout.flush()
