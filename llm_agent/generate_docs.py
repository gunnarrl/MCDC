...
# Adjust this to point to your MCDC source root
MCDC_SOURCE = Path("corpus/mcdc")
OUTPUT_JSON = Path("scraped_docs/auto_api.json")
...
def extract_from_node(node: ast.AST, pkg_name: str) -> Dict[str, Any]:
    """Pull name, docstring, params from function or class."""
    ...
    if isinstance(node, ast.FunctionDef):
        ...
        data["type"] = "class"
        # inspect __init__ for constructor params
        init = next(
...
def walk_package(pkg_path: Path) -> Dict[str, Any]:
    ...
    for py_file in pkg_path.rglob("*.py"):
        ...
        module_name = ".".join(py_file.relative_to(pkg_path.parent).with_suffix("").parts)
        try:
...
def main():
    ...
    rtd_funcs = {"material", "nuclide", "cell", "lattice", "surface", "universe", "eigenmode", "setting", "source", "tally"}
    auto_funcs = {k.split(".")[-1] for k in api.keys() if k.count(".") == 1}  # top-level only
