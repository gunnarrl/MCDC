...
PHYSICS_DOCS_DIR = CORPUS_ROOT / "physics_docs"
SOURCE_CODE_DIR = CORPUS_ROOT / "mcdc"
...
def infer_section_from_function(func_name: str) -> str:
    ...
    if 'material' in func_lower or 'nuclide' in func_lower:
        ...
        return 'tally'
    elif 'setting' in func_lower or 'eigenmode' in func_lower or 'particle' in func_lower or 'batch' in func_lower:
        return 'settings'
...
def extract_function_with_context(file_path: Path, node: ast.FunctionDef) -> Tuple[str, Dict]:
    """
    Extract function with surrounding context (modified from original).
    """
...
def chunk_code_semantically(file_path: Path, metadata_base: Dict) -> List[Document]:
    """
    Chunk code by logical units (functions/classes) with intelligent grouping.
    
    FIX: Now adds 'section' metadata to each chunk.
    """
    docs = []
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                if func_lines < 50:
                    ...
                    while j < len(nodes) and isinstance(nodes[j], ast.FunctionDef):
                        next_func = nodes[j]
                        next_lines = (next_func.end_lineno or next_func.lineno) - next_func.lineno
                    ...
                    metadata.update({
                        ...
                        "section": infer_section_from_function(func_names[0]), # ADDED
...
def load_source_code_enhanced() -> List[Document]:
    ...
    for py_file in py_files:
        ...
        metadata_base = {
            ...
            "quality": "high",
            "priority": 4 if not module_is_private else 3
            # 'section' will be added by chunk_code_semantically
        }
...
def load_pdf_documents_enhanced() -> List[Document]:
    ...
    FIX: Now adds 'section': 'theory' metadata
    """
    docs = []
...
def load_examples_enhanced() -> List[Document]:
    ...
    FIX: This now iterates through identified sections (material, surface, etc.)
    and creates one document per section, which is what the tutor expects.
    """
    docs = []
    ...
    for example_file in EXAMPLES_DIR.glob("*.py"):
        ...
        for section_name, section_lines in workflow_sections.items():
            ...
            metadata = {
                ...
                "type": "code_example",
                "test_name": test_name,
                "section": section_name,  # THIS IS THE CRITICAL FIX
                "complexity": complexity,
                "functions_used": ','.join(functions_used[:10]),
                "quality": "medium",
                "priority": 1
...
def identify_workflow_sections(content: str) -> Dict[str, List[str]]:
    ...
    # This pattern is from your original build_index.py
    section_patterns = {
        ...
        for func_name, data in rtd_data.items():
            ...
            docs.append(Document(
                ...
                metadata={
                    ...
                    "type": "api_doc",
                    "function": func_name,
                    "section": infer_section_from_function(func_name), # ADDED
                    "quality": "high",
                    "priority": 5
                }
            ))
...
def load_all_documents() -> List[Document]:
    """Load all documents with enhanced chunking."""
    docs = []
   
    # 1. API Docs (no chunking needed, section added)
    docs.extend(load_api_docs())
   
    # 2. Papers (page-by-page, section added)
    docs.extend(load_pdf_documents_enhanced())
   
    # 3. Source Code (semantic chunking, section added)
    docs.extend(load_source_code_enhanced())
   
    # 4. Examples (section chunking, section added)
    docs.extend(load_examples_enhanced())
   
    logging.info(f"\n{'='*60}")
    logging.info(f"Total documents: {len(docs)}")
    logging.info(f"  - API docs: {sum(1 for d in docs if d.metadata.get('type') == 'api_doc')}")
    logging.info(f"  - Papers: {sum(1 for d in docs if d.metadata.get('type') == 'paper')}")
    logging.info(f"  - Source code: {sum(1 for d in docs if 'code' in d.metadata.get('type', ''))}")
